...
Herdr plays a done sound when an agent finishes and an attention sound when an agent needs input. Sound notifications also emit a terminal bell so the host terminal can flash, bounce, or show its configured bell indicator. Set `enabled = false` on shared machines or remote servers unless you explicitly want audio or terminal bell alerts.
